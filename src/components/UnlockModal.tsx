      ...
      <div className="bg-white dark:bg-gray-800 w-full sm:w-[360px] sm:rounded-2xl rounded-t-2xl p-5 pb-8 sm:pb-5 animate-slide-up">
        {status === "confirm" && (
          <>
            ...
            <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-700 rounded-xl p-3 mb-4">
              <span className="text-sm text-gray-600 dark:text-gray-300">차감 포인트</span>
              <span className="text-sm font-bold text-orange-500">-5P</span>
            </div>
            <div className="flex items-center justify-between px-1 mb-5">
              <span className="text-xs text-gray-400 dark:text-gray-500">보유 포인트</span>
              <span className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">{userPoints}P → {userPoints - 5}P</span>
            </div>
            <div className="flex gap-2">
              <button
                onClick={onClose}
                className="flex-1 py-2.5 text-sm font-medium text-gray-600 bg-gray-100 dark:bg-gray-700 rounded-xl hover:bg-gray-200 transition"
              >
                취소
              </button>
              <button
        ...
        {status === "loading" && (
          <div className="flex flex-col items-center gap-3 py-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">맛집 정보를 가져오는 중...</p>
          </div>
        )}

        {status === "success" && (
          <div className="flex flex-col items-center gap-3 py-4">
            <div className="w-14 h-14 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center animate-bounce-once">
              <span className="text-2xl">🎉</span>
            </div>
            <h3 className="text-base font-bold text-gray-900 dark:text-gray-100">잠금 해제 완료!</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500">-5P 차감되었습니다</p>
            <button
              onClick={onClose}
        ...
        {status === "error" && (
          <div className="flex flex-col items-center gap-3 py-4">
            ...
            <h3 className="text-base font-bold text-gray-900 dark:text-gray-100">오류</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">{errorMsg}</p>
            <button
              onClick={onClose}
              className="mt-2 w-full py-2.5 text-sm font-medium text-gray-600 bg-gray-100 dark:bg-gray-700 rounded-xl hover:bg-gray-200 transition"
            >
              닫기
            </button>
          </div>
